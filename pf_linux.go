...
func GetPfs() ([]*Pf, error) {
	...
	for _, file := range files {
		...
		err = pf.PopulateVfIndexes()
		if err != nil {
			fmt.Printf("error getting vf indexes: %v\n", err)
		}
		pfs = append(pfs, &pf)
...
func (pf *Pf) PopulateVfIndexes() error {
	return PopulateVfIndexes(pf)
}

// virtfn0 -> ../0000:08:00.1
func PopulateVfIndexes(pf *Pf) error {
	pfLink, err := netlink.LinkByIndex(pf.Index)
	if err != nil {
		return err
	}
	vfMacMap := make(map[int]string)
	for _, vfInfo := range pfLink.Attrs().Vfs {
		vfMacMap[vfInfo.ID] = vfInfo.Mac.String()
	}
	re := regexp.MustCompile("virtfn([0-9]+)")
	pfDir, err := filepath.EvalSymlinks(fmt.Sprintf(pciDevicePath, pf.Address))
	if err != nil {
		return err
	}
	vfIndexes := make(map[string]int)
	walk := func(fn string, fi os.FileInfo, err error) error {
		...
		vfIndex, err := strconv.Atoi(re.FindStringSubmatch(fi.Name())[1])
		if err != nil {
			return err
		}
		v, err := filepath.EvalSymlinks(fmt.Sprintf("%s/%s", pfDir, fi.Name()))
		vfAddress := filepath.Base(v)
		if err != nil {
			return err
		}
		vfIndexes[vfAddress] = vfIndex
		return nil
	}
	filepath.Walk(pfDir, walk)
	for _, vf := range pf.Vfs {
		vf.PfIndex = pf.Index
		if _, ok := vfIndexes[vf.Address]; ok {
			vf.Index = vfIndexes[vf.Address]
			vf.MacAddress = vfMacMap[vf.Index]
		} else {
			vf.Index = -1
		}
	}
